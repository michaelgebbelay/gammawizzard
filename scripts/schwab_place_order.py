...
def to_schwab_opt(sym: str) -> str:
    ...
    # Already OCC-like: ROOT<=6 + YYMMDD + C/P + 8-digit
    ...
    # Already left-padded root (6 chars)
...
def parse_bid_ask(qobj: dict):
    """
    Extract bid/ask robustly from Schwab quote payloads.
    """
    if not qobj: return (None, None)
    # Some payloads are under 'quote', some not
...
def fetch_bid_ask(c, symbol: str):
    ...
    if r.status_code != 200:
        return (None, None)
    j = r.json()
    # API returns keyed by symbol you asked for
...
def compute_mid(c, legs_osi: list):
    """
    legs_osi = [buy_put, sell_put, sell_call, buy_call] as 21-char OCC symbols.
    mid = (netBid + netAsk)/2
      netBid = (sell_put.bid + sell_call.bid) - (buy_put.ask + buy_call.ask)
      netAsk = (sell_put.ask + sell_call.ask) - (buy_put.bid + buy_call.bid)
    """
...
def main():
    ...
    # ----- ladder knobs (safe defaults) -----
    TICK                = env_float("TICK", 0.05)
    DELAY_SEC           = env_int("REPRICE_DELAY_SEC", 30)
    MAX_STEPS           = env_int("MAX_REPRICE_STEPS", 8)        # total reprice iterations
    STICKY_MID_STEPS    = env_int("STICKY_MID_STEPS", 2)         # how many cycles we stick to mid before offsetting
    OFFSET_CREDIT       = env_float("OFFSET_CREDIT", 0.05)       # after sticky, credit = mid - OFFSET_CREDIT
    OFFSET_DEBIT        = env_float("OFFSET_DEBIT",  0.05)       # after sticky, debit  = mid + OFFSET_DEBIT
    MIN_CREDIT_START    = env_float("MIN_CREDIT_START", 2.00)    # guard only for the initial price
    MAX_DEBIT_START     = env_float("MAX_DEBIT_START",  1.90)    # guard only for the initial price
    CANCEL_REMAINING    = env_str("CANCEL_REMAINING", "true").lower() == "true"
    FALLBACK_NET_PRICE  = env_float("NET_PRICE", 0.05)           # used only if quotes fail
    ...
    # ensure schwab header and get sheetId
    ...
    # legs exactly as on sheet (we still have to convert to OCC 21-char for API)
    ...
    # de-dupe by raw legs already logged (placed/order)
    ...
    # convert to OCC 21-char for API
    try:
    ...
    # ---------- build the order dict (price set later) ----------
    ...
    # ---------- initial price from mid (with start guards), rounded to tick ----------
    mid, net_bid, net_ask = compute_mid(c, leg_syms)
    if mid is None:
        # quotes failed → fall back to NET_PRICE env
        start_price = FALLBACK_NET_PRICE
    else:
        start_price = mid

    if is_credit:
        # start guard: not below min credit start
        start_price = max(start_price, MIN_CREDIT_START)
    else:
        # start guard: not above max debit start
    ...
    try:
        ...
        try:
            ...
            # some implementations return Location header instead of JSON id
    ...
    # if initial place failed hard, log and exit
    if not (isinstance(status_code,int) and 200 <= status_code < 300):
        ...
        s.spreadsheets().values().update(
            ...
            body={"values":[[
                datetime.now(timezone.utc).isoformat(),
                "SCHWAB_ERROR",
                acct_hash, acct_last4, "SPXW", last_px,
                g("signal_date"), "PLACE", side, qty_exec, order_type, order["price"],
                raw_legs[0], raw_legs[1], raw_legs[2], raw_legs[3],
    ...
    def replace_price(oid: str, new_price: float):
        ...
        new_id = oid
        try:
            # Schwab may return a new orderId in body or Location header
            j = r.json()
    ...
    for step in range(MAX_STEPS):
        ...
        # Typical statuses: QUEUED, WORKING, FILLED, CANCELED, REJECTED
        ...
        if step < STICKY_MID_STEPS:
            ...
            if is_credit:
                target = mid - OFFSET_CREDIT
            else:
                target = mid + OFFSET_DEBIT
    ...
    # end of ladder: cancel if not filled and configured to do so
    final_status = None
    if not filled and CANCEL_REMAINING and current_id:
        ...
        # fetch a final status
        sc, st = get_order_status(current_id)
        final_status = st or status_code

    # ---------- log one final row at top for dedupe ----------
    # cosmetic symbol label from first raw leg
    try:
    ...
    s.spreadsheets().values().update(
        ...
        body={"values":[[
            ...
            g("signal_date"), "PLACE", side, qty_exec, order_type, price_used,
            raw_legs[0], raw_legs[1], raw_legs[2], raw_legs[3],
