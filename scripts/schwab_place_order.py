...
def get_option_buying_power(c, acct_hash: str):
    ...
    for section in ("currentBalances", "projectedBalances", "initialBalances", "balances"):
        b = sa.get(section, {})
        for k in (
            "optionBuyingPower", "optionsBuyingPower",
            "buyingPower", "cashAvailableForTrading",
            "availableFunds", "cashAvailableForTradingSettled"
        ):
            v = b.get(k)
...
def main():
    ...
    OFFSET_CREDIT = env_float("OFFSET_CREDIT", -0.05)   # signed: -0.05 = mid-0.05
    OFFSET_DEBIT  = env_float("OFFSET_DEBIT",  0.05)    # signed: +0.05 = mid+0.05
    ...
    if len(two) < 2:
        print("No leocross row 2; nothing to place.")
        sys.exit(0)
    h, r2 = two[0], two[1]
    idx = {n: i for i, n in enumerate(h)}
    def g(col):
        j = idx.get(col, -1)
        return r2[j] if 0 <= j < len(r2) else ""

    # guards: freshness and tomorrow expiry (to block "yesterday's trade")
    ts = parse_iso(g("ts"))
    ...
    put_w  = abs(strike_from_osi(leg_syms[1]) - strike_from_osi(leg_syms[0]))  # sellP - buyP
    call_w = abs(strike_from_osi(leg_syms[3]) - strike_from_osi(leg_syms[2]))  # buyC - sellC
    width = round(min(put_w, call_w), 3)

    # --- Cat-rule fallback for side ---
    credit_or_debit = (g("credit_or_debit") or "").lower()
    side_sheet = (g("side") or "").upper()
    if credit_or_debit in ("credit", "debit"):
        is_credit = (credit_or_debit == "credit")
    else:
        try:
    ...
    # qty from %BP (overrides sheet qty if SIZE_MODE=PCT_BP)
    qty_sheet = int((g("qty_exec") or "1"))
    qty_exec = qty_sheet
    if SIZE_MODE == "PCT_BP":
        bp = OPT_BP_OVERRIDE if OPT_BP_OVERRIDE and OPT_BP_OVERRIDE > 0 else get_option_buying_power(c, acct_hash)
        if bp is None:
    ...
    # Build order (price reprice later; qty fixed now)
    ...
    # ladder: sticky mid, then signed offsets
    filled = False
    current_id = order_id
