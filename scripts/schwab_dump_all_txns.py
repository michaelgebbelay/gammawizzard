#!/usr/bin/env python3
"""
Schwab → Sheets raw activity dump with execution enrichment + column formatting.

What’s new in this build (vs prior):
- Fixes Google Sheets "dates in price" issue by:
  * Using valueInputOption=RAW when writing values
  * Forcing column number formats: NUMBER for numeric cols, TEXT for the rest
- Keeps: non-overlapping ET chunking, 204 handling, order/execution enrichment, OCC parsing.
"""
...
# --- Columns (0-based) ---
TEXT_COLS    = [0,1,2,3,4,5,6,7,9,16]                 # strings
NUMERIC_COLS = [8,10,11,12,13,14,15]                  # numbers
...
def format_tab_columns(svc, sid: str, sheet_id: int, tab: str) -> None:
    # Determine a generous row span; use grid rowCount if available, else 100000.
    meta = svc.spreadsheets().get(spreadsheetId=sid).execute()
    row_count = 100000
    ...
    # Set TEXT format to avoid auto-date parsing on ts/ids/desc/etc
    for ci in TEXT_COLS:
        requests.append({
            "repeatCell": {
                "range": {
    ...
    # Set NUMBER format on numeric columns (strike/qty/price/amount/net/fees)
    # Pattern allows integers or decimals without scientific notation.
...
def write_rows_raw(svc, sid: str, tab: str, headers: List[str], rows: List[List[Any]]) -> None:
    svc.spreadsheets().values().update(
        spreadsheetId=sid,
        range=f"{tab}!A1",
        valueInputOption="RAW",                     # <-- no auto-convert
...
def fmt_et_with_colon(dt: datetime) -> str:
    s = dt.astimezone(ET).strftime("%Y-%m-%dT%H:%M:%S%z")
...
def parse_from_description(desc: str) -> Tuple[Optional[str], Optional[str], Optional[float], Optional[str]]:
    # e.g. "Removed due to Expiration PUT S & P 500 INDEX $6470 EXP 08/15/25"
...
# --- schwab-py exposes these methods; we rely on them (see docs) ---
def get_transaction_detail(c, acct_hash: str, txn_id: str) -> Optional[Dict[str, Any]]:
    try:
        r = c.get_transaction(acct_hash, txn_id)
...
def explode_txn(c, acct_hash: str, txn: Dict[str, Any]) -> List[List[Any]]:
    ...
    detail = get_transaction_detail(c, acct_hash, txn_id) if txn_id else None
    if detail:
...
def prepare_rows_for_sheet(rows: List[List[Any]]) -> List[List[Any]]:
    ...
    for r in rows:
        rr = list(r)
        # ensure text cols are strings (avoid accidental numeric parsing)
        for ci in TEXT_COLS:
            if ci < len(rr):
                rr[ci] = "" if rr[ci] in ("", None) else str(rr[ci])
        # ensure numeric cols are numbers (so formulas work)
        for ci in NUMERIC_COLS:
            if ci < len(rr):
                rr[ci] = _to_number(rr[ci])
        prepped.append(rr)
    return prepped
...
def main() -> int:
    ...
    # Prepare & write with RAW; fix formats to kill date rendering on price
    rows = prepare_rows_for_sheet(rows)
    clear_tab(svc, sid, RAW_TAB)
    format_tab_columns(svc, sid, sheet_id, RAW_TAB)   # set formats BEFORE writing
