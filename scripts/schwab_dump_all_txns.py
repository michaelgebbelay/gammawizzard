...
Fixes:
- TRADE-only.
- Dedup key includes the execution timestamp, so distinct fills at the
  same price/size are preserved.
- Credits positive: premium_gross = -Σ(amount).
- Adds contracts_abs (Σ|qty|) so 6 ICs show as 24 even if filled in 3× 2-lot chunks.
...
def leg_key(idx: Dict[str,int], row: List[str]) -> Tuple:
    """
    Uniqueness key for TRADE legs.
    IMPORTANT: include ts so distinct fills at same price/qty survive dedupe.
    """
    def g(name: str) -> str:
        i = idx.get(name, -1)
        return row[i] if 0 <= i < len(row) else ""
    txn_id = norm_str(g("txn_id"))
    ts     = norm_str(g("ts"))   # now the execution time from the dump
...
def main() -> int:
    ...
    # Keep TRADE-only snapshot from ACC
    ...
    # Dedupe RAW in-window (by leg_key that includes ts) and append only new
    tmp_seen = set()
    dedup_rows = []
    for r in raw_trade:
        k = leg_key(idx_raw, r)
    ...
    for r in acc_rows:
        ...
        amt    = to_float(g(r, "amount")) or 0.0   # SELLs negative (credit)
    ...
    for (exp, und) in sorted(agg.keys(), key=lambda b: (b[0], b[1]), reverse=True):
        ...
        legs = vals["legs"]                              # rows/fills count
        qty_net = round(vals["qty_net"], 6)              # expected ~0 for condors
        qty_abs = round(vals["qty_abs"], 2)              # e.g., 24 for 6 ICs
