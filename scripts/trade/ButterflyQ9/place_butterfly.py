#!/usr/bin/env python3
# BUTTERFLY Q9 — placer (3-leg ATM call butterfly)
#
# Reads BF_* env vars from orchestrator_bf5.py and places the butterfly via Schwab API.
#
# Ladder (same as CS):
#   BUY  butterfly: mid, mid+0.15, mid+0.30 (debit — pay more each rung)
#   SELL butterfly: mid, mid-0.15, mid-0.30 (credit — accept less each rung)
#
...
def log_row(row: dict):
    path = os.environ.get("BF_LOG_PATH", "logs/butterfly_q9_trades.csv")
...
def place_butterfly_with_ladder(
    ...
    if direction == "SELL":
        ladder_spec = [(0.00, False), (-LADDER_STEP, False), (-2.0 * LADDER_STEP, True)]
    else:
        ladder_spec = [(0.00, False), (+LADDER_STEP, False), (+2.0 * LADDER_STEP, True)]
