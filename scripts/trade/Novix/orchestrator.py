...
#
# Delegates placement to scripts/trade/ConstantStable/place.py via VERT_* envs.
...
def _init_events(today: date):
    ...
    if not _EVENTS_AVAILABLE:
        return None
    try:
        _ew = EventWriter(strategy="novix", account="schwab", trade_date=today)
        return _ew
...
# Placer script (shared with ConstantStable)
PLACER_SCRIPT = os.environ.get(
    "NX_PLACER_SCRIPT",
    "scripts/trade/ConstantStable/place.py",
)
...
def main():
    ...
    try:
        ...
        _close_events()
        print(f"{_TAG} SKIP: Schwab init failed: {e}")
        return 1
