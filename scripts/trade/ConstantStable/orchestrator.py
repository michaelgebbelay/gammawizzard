...
def main():
    ...
    # IC_LONG = both legs DEBIT (buying both verticals, needs big SPX move).
    # Decision pre-computed by ic_long_filter.py at 4:01 PM ET.
    is_ic_long = (v_put and v_call
                  and v_put["side"] == "DEBIT" and v_call["side"] == "DEBIT")

    if is_ic_long:
        from datetime import date as _date
        should_skip_ic, skip_reason = _read_ic_long_decision(_date.today().isoformat())
        if should_skip_ic:
