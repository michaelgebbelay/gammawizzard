#!/usr/bin/env python3
"""IC_LONG morning placer — reads deferred plan from S3, places at open.
...
Env:
  ...
  SCHWAB_APP_KEY, SCHWAB_APP_SECRET, SCHWAB_TOKEN_JSON — broker creds
"""
...
# Import from the existing Schwab placer
from schwab_token_keeper import schwab_client
from trade.ConstantStable.place import (
    bundle_nbbo,
...
def main():
    ...
    log_path = os.environ.get("CS_LOG_PATH", "/tmp/cs_trades.csv")
    ...
    try:
        ...
        print(f"{TAG}: broker init failed ({e})")
        return 1
    ...
    if mid > IC_PRICE_CAP:
        ...
        # wait_secs=0 means fire-and-forget (DAY order stays working)
        res = place_order_at_price(c, acct_hash, payload, qty,
        ...
        for rung, offset in enumerate(offsets):
            ...
            wait = STEP_WAIT if rung < len(offsets) - 1 else STEP_WAIT
            ...
            res = place_order_at_price(c, acct_hash, payload, remaining,
