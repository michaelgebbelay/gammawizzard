...
#  - Places a single 2‑leg SPX vertical (CREDIT or DEBIT)
#  - Uses 0.95 / 1.00 / 1.05 ladder, clamped into NBBO
#  - Logs trade details to CS_LOG_PATH
    ...
    if side == "CREDIT":
        ...
        legs = [
            {"instruction": "BUY_TO_OPEN",  "positionEffect": "OPENING",
             "quantity": qty, "instrument": {"symbol": long_osi,  "assetType": "OPTION"}},
            {"instruction": "SELL_TO_OPEN", "positionEffect": "OPENING",
             "quantity": qty, "instrument": {"symbol": short_osi, "assetType": "OPTION"}},
        ]
    else:  # DEBIT
...
def log_row(row: dict):
    ...
    cols = [
        ...
        "short_osi", "long_osi",
        "go", "strength", "is_strong",
        "unit_dollars", "oc", "units",
        "qty_requested", "qty_filled",
        "ladder_prices", "last_price", "nbbo_bid", "nbbo_ask", "nbbo_mid",
        "order_ids",
    ]
...
if __name__ == "__main__":
    ...
    acct_hash  = os.environ.get("SCHWAB_ACCT_HASH", "")

    STEP_WAIT        = float(os.environ.get("VERT_STEP_WAIT", "6"))
    MAX_LADDER_RUNGS = 3   # 0.95 / 1.00 / 1.05

    # Schwab client
    c = schwab_client()

    # NBBO for this vertical
    bid, ask, mid = vertical_nbbo(side, short_osi, long_osi, c)
    print(f"CS_VERT_PLACE {name}: side={side} kind={kind} short={short_osi} long={long_osi} qty={qty}")
    ...
    if not ladder:
        print("CS_VERT_PLACE: empty ladder after clamp — abort")
        sys.exit(0)
    ...
    print(f"CS_VERT_PLACE DONE {name}: qty_req={qty} qty_filled={filled} last_price={last_price}")
    sys.exit(0)
