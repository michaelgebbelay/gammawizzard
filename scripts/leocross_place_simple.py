...
QTY_TARGET = 4  # <<< daily target units
...
# Optional: set GUARD_ONLY=1 to print guard snapshot and exit without placing.
...
def to_osi(sym: str) -> str:
    ...
    raw = re.sub(r'[^A-Z0-9.$^]', '', raw)  # drop underscores etc.
...
def _osi_from_instrument(ins: dict) -> str | None:
    """Build a reliable OSI from Schwab instrument. Falls back to structured fields if symbol isn't parseable."""
    ...
    try:
        if exp and pc in ("CALL","PUT") and strike is not None:
            ...
            # root is irrelevant for canon; use 'SPXW ' as a neutral 6-char root
...
def gw_get_leocross():
    ...
    def hit(t):
        h={"Accept":"application/json","Authorization":"Bearer {}".format(_sanitize_token(t)),"User-Agent":"gw-orchestrator/1.2"}
...
def main():
    ...
    # Would this trade close any leg?
    blocked=None
    ...
    for _, osi, sign in checks:
        ...
        if (sign<0 and cur<0) or (sign>0 and cur>0):
            blocked = "WOULD_CLOSE {} acct_qty={:+g}".format(osi, cur)
            break

    if blocked:
        print("ORCH SKIP: {}".format(blocked))
        return 0
