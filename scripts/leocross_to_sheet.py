...
def compute_mid_condor(c, legs_osi: list):
    ...
    if None in (bp_bid,bp_ask,sp_bid,sp_ask,sc_bid,sc_ask,bc_bid,bc_ask): return (None,None,None,(None,)*8)
...
def main():
    ...
    # New optional guards
    MAX_LEG_SPREAD = env_float("MAX_LEG_SPREAD", None)     # e.g., 0.30 → if any leg ask-bid > 0.30, skip
    MAX_NET_SPREAD = env_float("MAX_NET_SPREAD", None)     # e.g., 0.25 → if (net_ask - net_bid) > 0.25, skip
    CANCEL_IF_UNFILLED = env_bool("CANCEL_IF_UNFILLED", False)

    # --- Schwab client from token file ---
    with open("schwab_token.json","w") as f: f.write(token_json)
    try:
        # Correct signature & kwarg per schwab-py docs
        c = client_from_token_file(token_path="schwab_token.json",
                                   api_key=app_key, app_secret=app_secret)
    except TypeError:
        # Fallback positional, in case of older lib shenanigans
        c = client_from_token_file("schwab_token.json", app_key, app_secret)
    ...
    if MAX_LEG_SPREAD is not None and None not in legs_ba:
        ...
        worst = max(
            bp_ask-bp_bid, sp_ask-sp_bid, sc_ask-sc_bid, bc_ask-bc_bid
        )
